...
#+END_SRC
** Ruby
setting은 [[https://worace.works/2016/06/07/getting-started-with-emacs-for-ruby/][다음]]을 참조했다.
*** ruby electric mode
#+BEGIN_SRC emacs-lisp
...
:IMAP설정:
   - [X] isync 설치
             local에서 gmail과 sync를 할 프로그램을 설치한다. brew install isync, 이렇게 설치하면 ~/.mbsyncrc를 설정할 수 있다.
             
   - [X] 아래는 ~/.mbsyncrc를 설정내용이다. 여길 [[file:~/.mbsyncrc][참조]]
     - (1)  *Mail폴더를 만든다.* home 아래, subFolder로 Gmail과 Fastmail을 만든다.  imap에서 가져올 메일이 여기에 저장된다. PATH와 INBOX설정에 기술한다. 그리고 mail server에서 imap을 설정해야 한다.
     ...
     - (2) *id&password처리*
       id는 user항목에 기술되어 있고 password는 2가지 방식이 있다. 아래 mail별 처리방식을 썼다.
       - Gmail
         - [IMAPAccount]-> [passCmd]: mail server에 login하기 위해선 id,pw가 필요하다. id는 위 항목중 [User]를 의미하고, pw는 [passCmd]를 사용했다. 지금은 pass는 보안문제로 사용되지 않는다. pass는 password를 직접 mbsyncrc파일에 기술하는 것이다. 그래서 나는  직접 기록하는 방법 대신에 특정파일에 적어놓고 이를 cat과 같은 명령어로 읽어들이는 방식을 사용했다. 그래서 [passCmd]를 사용했다.  gpg를 사용하는 방식도 있다. 여튼, 나는 cat을 사용했다. 이미 password는 ~/.oh-no-insecure-password에  적어놨다. 이 방식은 less insecure acess방식이라서 mail server설정이 필요하다. 이것은 아래에 기술했다.
       - Fastmail
         - fast mail은  app password를 받아서 사용했다. app password를 사용하는 방식은 아래에 기술했다.
       - 인증서
         인증서를 얻기 위해선, 아래와 같이 한다.
          #+begin_important
          $ mkdir ~/.cert
          $ openssl s_client -connect some.imap.server:port -showcerts 2>&1 < /dev/null | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' | sed -ne '1,/-END CERTIFICATE-/p' > ~/.cert/some.imap.server.pem
          ...
          $ openssl s_client -crlf -connect imap.gmail.com:993 -showcerts 2>&1 < /dev/null | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' | sed -ne '1,/-END CERTIFICATE-/p' > ~/.cert/imap.gmail.crt
          #+end_important
          이렇게 인증서를 얻고, CertificateFile에 기술한다. (.mbsyncrc를 참조)

   - [X] imap server 설정(less secure acess, app password) 
   ...
   - [X] 설정이 끝났으면 동기화 테스트를 한다.(mbsync -a)
     - [error] two pass인증 에러는 Application-specific password required란 에러나 나온다.
     ...
     - Maildir warning: ignoring INBOX in /Users/holy/Mail/Fastmail/
      ...
      - [error] credentials관련 error는 less secure access가 제대로 설정되지 않은 문제다.

:END:
:Mu설정:
   - [X] *mu를 설치한다. (brew install mu)*
     - mu를 system에 설치하면, mu4e도 설치가 된다. emacs에서 mu4e를 load할때는 아래 path를 사용해서 load한다. 그래서 emacs에서 설정해야 한다.
       #+begin_note
       (use-package mu4e)에서 load-path를 기술해야 한다.
       아래에 보면, load-path라는 항목이 있다. 설치 위치에 해당 패스를 기술한다.
   ...
   - [X] *mu init(예전 mu index)을  shell에서 실행한다.*
     저장된 mail에 indexing을 한다고 보면 된다. index tree를 만들어 검색을 빠르게 하는 것이다.
...
:SMTP설정:
- [X] SMTP server 설정
    - mu4e에서 mail계정은 context로 불리기도 한다. 각각의 계정마다 아래 smtp server 설정을 해준다. gmail과 fastmail에서 snmp정보를 찾을 수 있다. googling해서 찾아서 .emacs설정에 기입하면 된다.
...
- [X] mail 보내기 password문제. *.authinfo에 id,pw작성*
...
:end: 
:org에서메일보내는방법:
(1) org로 편지 쓰고, 파일 첨부하기
- org모드에서 글을 쓴다.
- 첨부할 파일을 선택한다. (M-x mml-attach-file) 
- C-c M-o (org-mime-org-buffer-htmlize)를 눌러서 보낸다.

(2) org subtree 메일 보내기
- org의 subtree에 커서를 위치한다.
- C-c M-s (org-mime-org-subtree-htmlize)로 보낸다.
:end:
:orgmail관련설정:
...
- mu4e에서 org 문서 작성후 htmlize해서 보내기
